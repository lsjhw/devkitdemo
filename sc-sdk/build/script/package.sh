...
function get_src_code() {
    ...
    git clone -b devkitdemo-23.0.0 https://github.com/lsjhw/devkitdemo.git
    if [[ $? -ne 0 ]]; then
...
function output() {
    ...
    if [[ ${OsArch} =~ "Redhat" ]]; then
        ...
        sha256sum kunpeng-sc-${PackageVersion}-1.aarch64.rpm > kunpeng-sc-${PackageVersion}-1.aarch64.rpm.sha256sum
        sha256sum kunpeng-sc-devel-${PackageVersion}-1.aarch64.rpm > kunpeng-sc-devel-${PackageVersion}-1.aarch64.rpm.sha256sum
    elif [[ ${OsArch} =~ "Debian" ]]; then
        ...
        sha256sum kunpeng-sc_${PackageVersion}_arm64.deb > kunpeng-sc_${PackageVersion}_arm64.deb.sha256sum
        sha256sum kunpeng-sc-devel_${PackageVersion}_arm64.deb > kunpeng-sc-devel_${PackageVersion}_arm64.deb.sha256sum
    fi
